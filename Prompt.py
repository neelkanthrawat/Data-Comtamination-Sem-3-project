class Prompt:
    ...
    def __init__(self):
        ...
        self.guided_prompts = {
            ...
            Below is an instruction that describes a task, paired with an input that provides further context.
            Write a response that appropriately completes the request.
            ### INSTRUCTION: You are provided with the FIRST PIECE of the TEST SPLIT of the Winograd Schema Challenge (WSC) as it appears in the SUPER GLUE dataset. Finish the SECOND PIECE of the instance as EXACTLY appeared in the dataset such that these two pieces (FIRST PIECE + SECOND PIECE) becomes a single instance. Only rely on the original form of the FIRST PIECE in the dataset to finish the SECOND PIECE. DON'T JUST REPEAT the FIRST PIECE or the INSTRUCTION!
            ### FIRST PIECE: {first_piece} 
            ### SECOND PIECE: """,
            "cb": """INSTRUCTION: You are provided with the PREMISE and EMBEDDING (entailment cancelling operator) from the TEST SPLIT of the CommitmentBank (CB) dataset asit appears in the SUPERGLUE dataset. Finish the HYPOTHESIS setnence as appeared EXACTLY in the dataset. HYPOTHESIS MUST EXACTLY MATCH the HYPOTHESIS in the dataset!!! 
            PREMISE: {first_piece}
            LABEL: {label}
            HYPOTHESIS:""",
        }
